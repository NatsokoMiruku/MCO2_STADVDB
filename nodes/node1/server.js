...
import node1Connection from './db.js';
...
app.get('/status', async (req, res) => {
    try {
        await node1Connection.authenticate();
        res.status(200).send('Node 1 is operational and connected to the database.');
    } catch (error) {
        res.status(500).send('Node 1 is not operational.');
    }
});
...
app.post('/transaction', async (req, res) => {
    ...
    try {
        const GameInformation = node1Connection.define('game_information', {
