...
import node2Connection from './db.js';
...
app.get('/status', async (req, res) => {
    try {
        await node2Connection.authenticate();
        res.status(200).send('Node 2 is operational and connected to the database.');
    } catch (error) {
        res.status(500).send('Node 2 is not operational.');
    }
});
...
app.post('/transaction', async (req, res) => {
    ...
    try {
        const GameInformation = node2Connection.define('game_information', {
